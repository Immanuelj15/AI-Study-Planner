...
import { CalendarDays, Sparkles, Clock, CheckCircle2, RefreshCw } from 'lucide-react';
      ...
      <div className="glass-card rounded-3xl p-6 lg:p-8 border border-slate-800 space-y-6">
        <div>
          <h1 className="text-2xl font-black text-slate-100 flex items-center gap-2">
            <CalendarDays className="w-6 h-6 text-brand-purple" /> Agent 4 Adaptive Scheduler
          </h1>
          <p className="text-slate-400 text-xs mt-1">
            Configure exam target and daily hours. Scheduler Agent adapts allocated hours based on quiz scores.
          </p>
        </div>

        <form onSubmit={handleGeneratePlan} className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-300">Exam Target Date</label>
            <input
              type="date"
              required
              value={examDate}
              onChange={(e) => setExamDate(e.target.value)}
              className="w-full glass-input py-2 px-3 rounded-xl text-xs"
            />
          </div>

          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-300">Daily Study Hours</label>
            <input
              type="number"
              step="0.5"
              min="1"
          ...
          <div className="flex items-end">
            <button
              type="submit"
              disabled={generating}
              className="w-full py-2.5 px-4 rounded-xl gradient-btn text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-brand-500/20"
            >
      ...
      {plans.length > 0 ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between text-xs font-bold text-slate-400">
            <span>Generated Adaptive Matrix ({plans.length} sessions)</span>
            <span className="text-brand-cyan">Sorted by Priority & Date</span>
          </div>
